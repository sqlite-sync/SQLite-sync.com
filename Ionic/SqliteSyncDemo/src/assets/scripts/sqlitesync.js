			...
			if(response.trim().length != 0)
			{
				...
				sqlitesync_DB.transaction(function (tx) {
					...
					if (responseReturn[0].SyncId > 0) {
						...
						sqlitesync_AddLog('Preparing changes for table <b>' + responseReturn[0].TableName + '</b>');
					...
					if(syncId > 0){
						$.ajax({
							...
							timeout: 5 * 60 * 1000,//10min
							success: function(){
								sqlitesync_AddLog('Received the table '+tableNameSync+'.');
							...
							error: function (result, request) {
								...
								sqlitesync_AddLog('Error while syncing with the server ' + error);
								sqlitesync_SyncTableCurrentIndex++
...
function sqlitesync_SyncSendData() {
    sqlitesync_AddLog('Starting sending data');
...
function sqlitesync_ReinitializeDB(syncServer, syncPdaIdent) {
    ...
    $.ajax({
                      ...
                      .forEach(function(v, i) {
                          if(i != 0){
                                tx.executeSql(responseReturn[v],
                                    ...
                                    function (transaction, error) {
                                        sqlitesync_AddLog('Object ' + v  + '; ' + error.message + ' (Code ' + error.code + ')');
                                    });
                          }
                       });
        ...
        error: function (result, request) {
            ...
            sqlitesync_AddAlert('Error while syncing with the server - ' + statusText);
        }
    });
}
